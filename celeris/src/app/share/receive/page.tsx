  ...
  const startScanner = useCallback(async () => {
    ...
    try {
      ...
      if (
        ...
        userMessage =
          "No camera found. Make sure your device has a camera connected.";
      } else if (
              ...
              {(uiState === ConnectionUIState.CONNECTED ||
                ...
                connectionId && (
                  <span className="font-bold text-muted-foreground text-xs sm:text-sm">
                    Connection ID:{" "}
                    <span className="text-primary">{connectionId}</span>
                  </span>
                )}
              <Badge
            ...
            <div className="relative flex flex-col items-center justify-center px-4 sm:px-6 lg:px-8 rounded-xl sm:rounded-2xl w-full h-[50vh] sm:h-[55vh] lg:flex-1 lg:max-h-70 bg-card border border-border hover:border-border/80 transition-all duration-500 overflow-hidden">
              {receivedFiles.length > 0 ? (
                <ScrollArea className="w-full h-[40dvh] sm:h-[50dvh] lg:h-full lg:max-h-[calc(100vh-30rem)]">
                  ...
                  {uiState === ConnectionUIState.RECEIVING &&
                    fileReceiverRef.current?.fileInfo &&
                    (() => {
                      const currentFile = fileReceiverRef.current.fileInfo;
                      const progress = currentFile
                        ? (downloadProgress[currentFile.name] ?? 0)
                        : 0;
                      return (
                        <div className="w-full px-1 py-3 border-b border-border/50">
                          <div className="flex items-center gap-2 mb-2">
                            <Download className="h-4 w-4 text-primary animate-bounce" />
                            <span className="text-xs sm:text-sm font-medium text-primary truncate max-w-[200px] sm:max-w-[300px]">
                              {currentFile?.name}
                            </span>
                ...
                <div className="flex flex-col items-center justify-center h-full">
                  {uiState === ConnectionUIState.IDLE ? (
                    ...
                    <div className="flex flex-col items-center justify-center gap-4 px-4 sm:px-8 w-full max-w-md">
                      <Download className="w-10 h-10 sm:w-12 sm:h-12 text-primary animate-bounce" />
                          ...
                          return (
                            <div className="w-full space-y-3">
                              <div className="flex items-center justify-between">
                                <span className="text-xs sm:text-sm text-foreground font-medium truncate max-w-[180px] sm:max-w-[260px]">
                                  {currentFile.name}
                                </span>
                              ...
                              <p className="text-xs text-muted-foreground text-center">
                                ...
                                of {formatFileSize(currentFile.size)}
                              </p>
                            </div>
                          );
                        })()}
      ...
      <Dialog open={isConnectDialogOpen} onOpenChange={setConnectDialogOpen}>
        <DialogContent className="sm:max-w-xl w-[95vw] bg-background text-foreground border-none p-3 sm:p-6">
          ...
          <div className="space-y-4 my-2">
            ...
            {/* QR Scanner Video */}
            <div
              className={`overflow-hidden rounded-lg transition-all duration-300 ${isScanning || scannerError ? "max-h-[350px] opacity-100" : "max-h-0 opacity-0"}`}
            >
              {scannerError ? (
